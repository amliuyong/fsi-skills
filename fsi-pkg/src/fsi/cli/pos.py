...
from fsi.ai.prompts import POS_SYSTEM_PROMPT, build_user_message


@click.command("pos")
@click.pass_context
def pos_cmd(ctx):
    """持仓分析（逐股深度 + 仓位结构 + AI 评级）"""
    ...
    market_ctx = fetch_market_context(
        include_calendar=False, verbose=ctx.obj.get("verbose", False)
    )
    ...
    # AI 分析
    click.echo("正在生成 AI 持仓分析...", err=True)
    try:
        ai_text = call_bedrock(POS_SYSTEM_PROMPT, build_user_message(result))
        result["ai_analysis"] = ai_text
    except Exception as e:
        click.echo(f"AI 分析失败: {e}", err=True)
        result["ai_analysis"] = None

    output(result, fmt, title="持仓分析")
